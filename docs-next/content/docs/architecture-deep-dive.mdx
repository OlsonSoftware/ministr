...
description: In-depth walk through the ministr workspace, ingestion pipeline, code intelligence (symbols, references, bridges), search backends, and session tracking.
---
...
> detection — backed by session tracking, predictive prefetching, and coherence.
>
...
Think of ministr as a **smart librarian** that sits between an LLM agent and a codebase. Instead of the agent grepping and reading files blindly, ministr parses everything into an AST, answers in terms of symbols, references, and language bridges, and shapes its own output — resolution and granularity — so the agent's finite window stays full of signal instead of redundant re-reads.
