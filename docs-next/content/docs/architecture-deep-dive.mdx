...
Think of ministr as a **smart librarian** that sits between an LLM agent and a codebase. Instead of the agent naively reading files and losing track of what it's already seen, ministr indexes everything, tracks what's been delivered, predicts what's needed next, and shapes its own output — resolution, compression, deduplication — so the agent's finite window stays as clear as a well-managed L1 cache.
...
main()
  ...
  │
  ├─ 3. acquire_role()
  │     ├─ Try to grab a file lock on ~/.ministr/corpora/<hash>/ministr.lock
...
```text
SQLite Database (~/.ministr/corpora/<hash>/content.db)
  │
...
After each `ministr_read`, the prefetch engine predictively pre-warms sections the agent is likely to request next. **Three strategies fire today**, and two more are written but not yet wired up post-read:

**Live (run after every read):**
...
**In code but not yet triggered post-read:**

4. **Cross-Session** (shared cache) — pre-warm frequently co-accessed sections. Needs per-section co-access analytics in the storage layer first
...
ministr indexes content at multiple granularity levels and delivers at the appropriate resolution based on context budget pressure:
...
Each ministr MCP tool maps to a method chain:

| MCP Tool | `MinistrServer` method | `QueryService` method |
|---|---|---|
| `ministr_survey` | `handle_survey()` | `survey_excluding()` |
| `ministr_read` | `handle_read()` | `read_section()` |
| `ministr_extract` | `handle_extract()` | `extract_claims()` |
| `ministr_symbols` | `handle_symbols()` | `search_symbols()` |
| `ministr_definition` | `handle_definition()` | `symbol_definition()` |
| `ministr_references` | `handle_references()` | `symbol_references()` |
| `ministr_toc` | `handle_toc()` | `table_of_contents()` |
| `ministr_budget` | `handle_budget()` | *(session state only)* |
| `ministr_compress` | `handle_compress()` | `compress_for_eviction()` |
| `ministr_evicted` | `handle_evicted()` | *(session state only)* |
| `ministr_fetch` | `handle_fetch()` | `WebFetcher` + ingest |
| `ministr_clone` | `handle_clone()` | `GitFetcher` + ingest |
| `ministr_refresh` | `handle_refresh()` | `WebFetcher` staleness check |
| `ministr_bridge` | `handle_bridge()` | `search_bridge_links()` |
| `ministr_related` | `handle_related()` | `related_claims()` |
