...
flowchart TB
  ...
  subgraph engine["ministr — local code-intelligence engine"]
    ...
    sess["Session tracking<br/>skip re-sends · ship deltas"]
    index[("Local index")]
    intel --> index
    retr --> index
  end
...
Underneath both, **session tracking** is a quiet efficiency layer: ministr remembers what it has already shown the agent this session, so a repeat request comes back as a pointer or just the lines that changed. It is a bonus, not the headline.
...
- [Core concepts](/docs/concepts) — the ideas behind semantic retrieval, session tracking, and coherence, in plain terms.
