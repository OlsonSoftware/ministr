...
# Scoped daemon data dir + config — so the demo NEVER touches the
# user's real `~/.ministr/`. Default is `/tmp/ministr-demo-data`,
# wiped on each run. Set DEMO_DATA_DIR=keep-me to keep state
# across runs (faster restart at cost of fast-skip).
DEMO_DATA_DIR="${DEMO_DATA_DIR:-/tmp/ministr-demo-data}"
DEMO_CONFIG="${DEMO_CONFIG:-/tmp/ministr-demo-config.toml}"
...
# Scope the daemon's data dir to /tmp via a one-off config file —
# the demo MUST NOT touch ~/.ministr/ (which holds the user's real
# Tauri-app state). Each run wipes the scoped dir first so there's
# no stale corpora state to interfere.
if [[ "${DEMO_DATA_DIR}" != "keep-me" ]]; then
    info "wiping scoped demo data dir ${DEMO_DATA_DIR}"
    rm -rf "${DEMO_DATA_DIR}"
fi
mkdir -p "${DEMO_DATA_DIR}"
cat > "${DEMO_CONFIG}" <<EOF
# Auto-generated by scripts/demo-local.sh; safe to delete.
...
[prefetch]
EOF
info "scoped data_dir=${DEMO_DATA_DIR} (config at ${DEMO_CONFIG})"
...
if [[ -z "${CORPUS_ID}" ]]; then
    ...
    echo >&2
    echo "--- last 20 lines of ${SERVE_LOG} for context ---" >&2
    tail -20 "${SERVE_LOG}" >&2
    echo "--- end log ---" >&2
    echo >&2
    echo "If this is a filesystem error in ~/.ministr/, your Tauri app's"  >&2
    echo "auto-spawned daemon may be holding state there. The demo runs"  >&2
    echo "in a scoped /tmp/ministr-demo-data dir specifically to avoid"  >&2
    echo "that — check ${SERVE_LOG} for clues about which dir blew up." >&2
    exit 1
fi
...
info "daemon data dir (scoped): ${DEMO_DATA_DIR}"
info "blob backend root: ${MINISTR_BLOB_FS_ROOT}"
info "your real ~/.ministr/ was NOT touched"
