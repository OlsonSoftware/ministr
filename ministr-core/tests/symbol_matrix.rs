...
fn extract_fixture(lang: &str, ext: &str) -> Vec<ministr_core::code::Symbol> {
...
fn assert_has_symbols(lang: &str, symbols: &[ministr_core::code::Symbol], expected: &[Expected]) {
    for exp in expected {
        let found = symbols
            .iter()
