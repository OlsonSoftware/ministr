...
async fn migration_rollforward() {
    use ministr_core::storage::CURRENT_SCHEMA_VERSION;
...
async fn web_cache_crud() {
    use ministr_core::storage::WebCacheRecord;
...
async fn git_cache_upsert_and_get() {
    use ministr_core::storage::GitCacheRecord;
    ...
    let record = GitCacheRecord {
        ...
        clone_dir: "/tmp/ministr/remote/abcdef1234567890".into(),
...
fn sample_symbols() -> Vec<SymbolRecord> {
    vec![
        ...
        SymbolRecord {
            ...
            kind: "function".into(),
            visibility: "pub".into(),
            signature: "pub async fn run(config: &MinistrConfig) -> Result<()>".into(),
...
async fn insert_and_get_symbol() {
    ...
    let sym = storage
        .get_symbol(&SymbolId("sym-config::MinistrConfig".into()))
        .await
        .unwrap();
    assert!(sym.is_some());
    let sym = sym.unwrap();
    assert_eq!(sym.name, "MinistrConfig");
    ...
    assert_eq!(
        ...
        Some("Configuration for ministr.")
    );
}
...
async fn insert_and_query_symbol_refs() {
    ...
    // Query from the target side — MinistrConfig should appear in Uses refs
    let uses = storage
        .query_refs(
...
async fn delete_refs_for_file_cascades() {
    ...
    let remaining = storage
        .query_refs(&SymbolId("sym-config::MinistrConfig".into()), None)
        .await
        .unwrap();
    assert!(remaining.is_empty());
}
...
async fn list_documents_by_root_filters_correctly() {
    ...
    let doc2 = ministr_core::types::DocumentTree {
        id: ministr_core::types::ContentId("doc-second".into()),
...
async fn corpus_root_provenance_roundtrip() {
    ...
    let root = CorpusRoot {
        id: "root-git1".into(),
        path: "/home/user/.ministr/remote/abc".into(),
        kind: RootKind::Git,
