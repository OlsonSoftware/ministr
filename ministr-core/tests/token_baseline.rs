...
//! concatenation vs ministr-style targeted retrieval.
//!
...
/// Simulate an ministr-style workflow: deliver only relevant sections/claims.
///
/// Returns the total token count of targeted extracts.
fn ministr_targeted_tokens(extracts: &[&str]) -> usize {
...
/// Section-level context ministr would deliver (the `SessionManager` struct + impl).
const MINISTR_SECTION: &str = r"
pub struct SessionManager {
    sessions: HashMap<String, Session>,
    timeout: Duration,
}
...
fn ministr_uses_fewer_tokens_than_raw_cat_for_targeted_query() {
    ...
    // ministr workflow: survey returns FILE_A section, extract returns cleanup fn
    let ministr_tokens = ministr_targeted_tokens(&[MINISTR_SECTION, MINISTR_EXTRACT]);

    // ministr should use significantly fewer tokens
    let savings_pct = 1.0 - (ministr_tokens as f64 / raw_tokens as f64);

    assert!(
        ministr_tokens < raw_tokens,
        "ministr should use fewer tokens: ministr={ministr_tokens}, raw={raw_tokens}"
    );
    assert!(
        savings_pct > 0.5,
        "ministr should save at least 50% of tokens: savings={savings_pct:.1}%"
    );
}
...
fn token_savings_scale_with_codebase_size() {
    ...
    assert!(
        savings_pct > 0.90,
        "with 10 files, ministr should save >90% tokens: savings={savings_pct:.2}"
    );
}
