...
use ministr_core::parser::ParserKind;
use ministr_core::storage::{SqliteStorage, Storage};
use ministr_core::web::cache::{WebCache, WebPageMeta, url_hash};
use ministr_core::web::fetcher::web_source_path;

/// Sample markdown content simulating a fetched web page.
const SAMPLE_MARKDOWN: &str = "\
# Getting Started with Ministr

Ministr is a context cache for LLM agents.

## Installation

Install ministr using cargo:
...
Create a configuration file at `~/.ministr/config.toml`:
...
Run ministr with a corpus directory:

```bash
ministr serve --corpus ./docs
```
...
async fn ingest_multiple_web_pages() {
    ...
    let pages = vec![
        (
            ...
            "# Introduction\n\nMinistr provides a multi-resolution index for LLM context management. It supports document summaries, section text, and atomic claims.\n",
        ),
        (
            "web://example.com/docs/api",
            "# API Reference\n\n## Survey\n\nThe ministr_survey tool searches the corpus for relevant content. It returns ranked results with scores.\n\n## Read\n\nThe ministr_read tool retrieves full section text by section ID.\n",
        ),
    ];
