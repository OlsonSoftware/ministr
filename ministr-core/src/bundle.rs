//! Index bundle export/import for portable `.ministr-index` files.
//!
...
//! - `index/ministr_hnsw.hnsw.dat` — HNSW graph data
//! - `index/ministr_hnsw.hnsw.graph` — HNSW connectivity graph
//!
...
/// File extension for ministr index bundles.
pub const BUNDLE_EXTENSION: &str = "ministr-index";
...
/// Import an `.ministr-index` bundle into a corpus directory.
///
...
///
/// ```
/// use ministr_core::bundle::{BundleCorpusRoot, compute_bundle_version};
///
...
/// Read only the manifest from a `.ministr-index` bundle without extracting files.
///
...
mod tests {
    ...
    fn export_missing_db_returns_error() {
        ...
        let output = tmp.path().join("test.ministr-index");
    ...
    fn export_import_roundtrip() {
        ...
        fs::write(index_dir.join("ministr_hnsw.hnsw.dat"), b"fake-dat").unwrap();
        fs::write(index_dir.join("ministr_hnsw.hnsw.graph"), b"fake-graph").unwrap();
        ...
        let bundle_path = tmp.path().join("test.ministr-index");
        ...
        assert!(import_dir.join("index/ministr_hnsw.hnsw.dat").exists());
        assert!(import_dir.join("index/ministr_hnsw.hnsw.graph").exists());
    }
    ...
    fn import_rejects_unsafe_paths() {
        ...
        let bundle_path = tmp.path().join("evil.ministr-index");
    ...
    fn import_rejects_future_version() {
        ...
        let bundle_path = tmp.path().join("future.ministr-index");
        let file = File::create(&bundle_path).unwrap();
        let writer = BufWriter::new(file);
        let zstd_writer = zstd::Encoder::new(writer, 1).unwrap();
        let mut archive = tar::Builder::new(zstd_writer);
    ...
    fn read_manifest_from_bundle() {
        ...
        fs::write(index_dir.join("ministr_hnsw.hnsw.dat"), b"dat").unwrap();
        fs::write(index_dir.join("ministr_hnsw.hnsw.graph"), b"graph").unwrap();
    ...
    fn read_manifest_missing_returns_error() {
        ...
        let bundle_path = tmp.path().join("no-manifest.ministr-index");
        let file = File::create(&bundle_path).unwrap();
        let writer = BufWriter::new(file);
        let zstd_writer = zstd::Encoder::new(writer, 1).unwrap();
        let mut archive = tar::Builder::new(zstd_writer);
    ...
    fn cache_entry_roundtrip() {
        let tmp = TempDir::new().unwrap();
        let entry = BundleCacheEntry {
            url: "https://example.com/test.ministr-index".into(),
