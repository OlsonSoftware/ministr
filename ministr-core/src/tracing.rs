...
/// The environment variable that controls log filtering (e.g. `RUST_LOG=ministr_core=debug`).
const ENV_FILTER_VAR: &str = "RUST_LOG";

/// The environment variable that selects output format (`json` or `pretty`).
const LOG_FORMAT_VAR: &str = "MINISTR_LOG_FORMAT";

/// Default filter when `RUST_LOG` is not set.
const DEFAULT_FILTER: &str =
    "ministr_core=info,ministr_mcp=info,ministr_daemon=info,ministr_cli=info,warn";
