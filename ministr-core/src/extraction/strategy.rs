...
mod tests {
    ...
    fn structured_claims_extracts_assertions() {
        ...
        let text = "In this section we discuss the compression pipeline. \
                     The ministr MCP server uses 128-dimensional ONNX embeddings for semantic search. \
    ...
    fn quality_good_compression() {
        let original = "The ministr MCP server uses 128-dimensional ONNX embeddings for semantic search. \
        ...
        let compressed = "ministr MCP server uses ONNX embeddings. Borrow checker ensures safety.";
