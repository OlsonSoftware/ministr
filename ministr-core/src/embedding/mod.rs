...
mod fastembed_impl;
pub mod hybrid;
// Azure OpenAI embedder moved here from ministr-cloud
...
///
/// ```
/// use ministr_core::embedding::{check_model_compatibility, ModelCompatibility};
///
...
pub fn create_embedder(
    ...
    let model_name = if prefer_quantized {
        if let Some(q) = suggest_quantized_model(model_name) {
            tracing::info!(
                original = model_name,
                quantized = q,
                "MINISTR_PREFER_QUANTIZED=1 — using quantized model"
            );
            q
        } else {
            model_name
    ...
    match backend_env.as_str() {
        "candle" => {
            #[cfg(feature = "candle")]
            {
                tracing::info!(model = %model_name, "using Candle backend (MINISTR_BACKEND=candle)");
            ...
            {
                tracing::warn!(
                    "MINISTR_BACKEND=candle requested but candle feature not enabled, falling back to ONNX"
                );
            }
        }
        "onnx" | "fastembed" => {
            tracing::info!(model = %model_name, "using ONNX/FastEmbed backend (MINISTR_BACKEND={backend_env})");
        }
        "" => {
            ...
            if force_cpu && is_candle_model(model_name) {
                tracing::info!(
                    model = %model_name,
                    "MINISTR_DEVICE=cpu — using ONNX backend instead of Candle Metal"
                );
            }
            ...
            if let Some(q) = suggest_quantized_model(model_name) {
                tracing::info!(
                    model = %model_name,
                    quantized = q,
                    "tip: use '{q}' or set MINISTR_PREFER_QUANTIZED=1 for 2-3x faster CPU inference"
                );
            }
        ...
        other => {
            tracing::warn!(
                backend = other,
                "unknown MINISTR_BACKEND value, falling back to ONNX"
            );
        }
    }
...
///
/// ```no_run
/// use ministr_core::embedding::{Embedder, FastEmbedder};
///
