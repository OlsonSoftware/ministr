...
/// All embedding models supported by ministr via fastembed.
///
...
/// configurable directory (defaults to `~/.ministr/models/`).
///
...
impl FastEmbedder {
    ...
    pub fn new(model_name: &str, cache_dir: Option<&str>) -> Result<Self, IndexError> {
        ...
        {
            let coreml_disabled =
                std::env::var("MINISTR_COREML").is_ok_and(|v| v == "0" || v == "false");

            if coreml_disabled {
                info!("CoreML disabled (MINISTR_COREML=0), using CPU execution provider");
            } else {
                let compute_units = match std::env::var("MINISTR_COMPUTE_UNITS")
                    .unwrap_or_default()
                    .as_str()
                {
...
///
/// ```no_run
/// use ministr_core::embedding::{DualEmbedder, Embedder, FastEmbedder, MatryoshkaEmbedder};
/// use std::sync::Arc;
///
/// let inner = Arc::new(FastEmbedder::new("nomic-embed-text-v1.5", None)?);
