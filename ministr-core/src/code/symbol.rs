...
///
/// ```
/// use ministr_core::code::Visibility;
///
...
///
/// ```
/// use ministr_core::code::{Symbol, Visibility, ItemKind};
///
/// let sym = Symbol {
...
///
/// ```
/// use ministr_core::code::{AstParser, extract_symbols, ItemKind, Visibility};
///
...
mod tests {
    ...
    // Test against real ministr-core source files
    ...
    fn extract_symbols_from_config_rs() {
        ...
        // Should have MinistrConfig struct
        let ministr_config = symbols
            .iter()
            .find(|s| s.name == "MinistrConfig" && s.kind == ItemKind::Struct);
        assert!(
            ministr_config.is_some(),
            "expected MinistrConfig struct, found: {:?}",
            symbols.iter().map(|s| &s.name).collect::<Vec<_>>()
        );
        let ic = ministr_config.unwrap();
        assert_eq!(ic.visibility, Visibility::Public);
        assert!(
            ic.doc_comment.is_some(),
            "MinistrConfig should have a doc comment"
        );
        assert!(ic.signature.contains("MinistrConfig"));
