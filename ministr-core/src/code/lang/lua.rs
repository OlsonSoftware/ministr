//! Lua language refinement.

use super::LanguageRefinement;
use crate::code::ast_parser::ItemKind;

/// Lua language refinement.
pub struct LuaRefinement;

impl LanguageRefinement for LuaRefinement {
    fn classify_node_kind(&self, kind: &str) -> Option<Option<ItemKind>> {
        let result = match kind {
            "function_declaration" | "function_definition" => Some(ItemKind::Function),
            "comment" => None,
            _ => return None,
        };
        Some(result)
