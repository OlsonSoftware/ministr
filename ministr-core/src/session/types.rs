...
pub enum CompressionTier {
    ...
    Extractive,
    /// Heading-only bookmark (~95%+ reduction). Re-fetchable via `ministr_read`.
    Bookmark,
...
impl Session {
    ...
    #[must_use]
    pub fn new(id: SessionId, agent_context_budget: usize, eviction_policy: DropPolicy) -> Self {
        Self {
            id,
    ...
    /// calls (e.g. `ministr_survey`, `ministr_symbols`) still register as
    ...
    /// The content remains re-fetchable via `ministr_read`.
    ///
