...
//! # Lib + bin shape (F31.3)
//!
//! The crate exposes `[lib]` so the private-repo `ministr-cli-cloud`
//! binary can call [`run`] without duplicating the ~750-line CLI
//! surface. The in-tree `ministr-cloud-tools` binary in `src/bin/`
//! reduces to a thin `#[tokio::main]` wrapper over the same call.
...
/// Entry point shared by both the in-tree `ministr-cloud-tools` binary
/// and the private-repo `ministr-cli-cloud` binary (F31.3). Parses argv,
/// installs the rustls `CryptoProvider` + miette hook + tracing, then
/// dispatches the parsed subcommand.
///
/// Callers are expected to provide their own `#[tokio::main]` runtime
/// (or equivalent) since `run` is just an `async fn`.
///
/// # Errors
///
/// Returns whatever `dispatch` returns — typically a `miette::Error`
/// from a failed cloud subcommand (Postgres connect, blob upload, JWT
/// signing, etc.). The miette hook is installed before dispatch so
/// error rendering is fancy.
///
/// # Panics
///
/// Panics if the miette hook fails to install — which only happens if
/// some other code installed a different miette hook first, and we
/// expect to be the first caller for any binary that wraps this fn.
