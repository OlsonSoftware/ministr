...
description: Generate, mint, audit, revoke, and rotate Enterprise license JWTs for on-prem deployments.
---
...
The CLI covers the full lifecycle: keypair generation, per-customer
JWT minting, revocation, key rotation, and audit logging.
...
2. **Customer side** — customers set
   `MINISTR_LICENSE_REVOCATIONS_URL=https://mcp.ministr.ai/api/v1/license-revocations.jsonl`
   and the boot validator fetches, caches, and falls back within the
   grace window. The file-based `MINISTR_LICENSE_REVOCATIONS=/path`
   flow also works for air-gapped deployments.
