//! Cloud bundle fetch — download and import remote `.iris-index` bundles.
//!
...
use iris_core::bundle::{self, BundleCacheEntry, BundleManifest};
use iris_core::config::CloudInclude;
...
async fn fetch_one(
    ...
    let tmp_path = tmp_dir.path().join("bundle.iris-index");
