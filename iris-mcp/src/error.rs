...
//!
//! These errors wrap iris-core error types and add `miette::Diagnostic`
//! metadata for user-facing error reports with codes, help text, and context.

use miette::Diagnostic;
use thiserror::Error;

use iris_core::error::{IndexError, ParseError, SessionError, StorageError};
...
pub enum McpError {
    ...
    #[error(transparent)]
    #[diagnostic(code(iris::mcp::index))]
    Index(#[from] IndexError),

    /// A session tracking operation failed.
    #[error(transparent)]
    #[diagnostic(code(iris::mcp::session))]
    Session(#[from] SessionError),

    /// A storage operation failed.
    #[error(transparent)]
    #[diagnostic(code(iris::mcp::storage))]
    Storage(#[from] StorageError),

    /// A document parsing operation failed.
    #[error(transparent)]
    #[diagnostic(code(iris::mcp::parse))]
    Parse(#[from] ParseError),

    /// An MCP protocol-level error (e.g. invalid tool parameters).
    #[error("MCP protocol error: {reason}")]
    #[diagnostic(code(iris::mcp::protocol), help("check the tool call parameters"))]
