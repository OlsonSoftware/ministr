...
export function Settings({
  ...
  return (
    <div className="space-y-4 iris-fade-in max-w-2xl">
      <header>
        <h2 className="text-base font-semibold text-text">Settings</h2>
        <p className="text-xs text-text-dim mt-0.5">
          Preferences for the iris desktop app and daemon.
        </p>
      </header>

      <Section icon={Power} title="Startup" description="Launch the iris tray app automatically at login.">
        <ToggleRow
      ...
      <Section
        icon={HardDrive}
        title="Storage"
        description="Where iris keeps its index and session data."
      >
        <Row label="Memory (RSS)" value={`${status.memory_mb.toFixed(0)} MB`} mono />
        <Row label="Data directory" value="~/.iris/" mono />
      </Section>
      ...
      >
        <div className="flex items-center justify-between gap-4">
          <p className="text-xs text-text-muted">
            Useful after adding <span className="font-mono">.iris.toml</span>{" "}
