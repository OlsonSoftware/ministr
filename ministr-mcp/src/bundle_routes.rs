...
//!
//! When ministr runs as a remote MCP server, these routes let clients download
...
use ministr_core::index::{VectorIndex as _, VectorIndexLoad as _};
...
fn version_headers(headers: &mut HeaderMap, manifest: &BundleManifest) {
    ...
    if let Some(ref sha) = manifest.source_commit {
        let _ = headers.insert(
            "X-Ministr-Source-Commit",
            HeaderValue::from_str(sha).unwrap_or_else(|_| HeaderValue::from_static("")),
        );
    }
    let _ = headers.insert(
        "X-Ministr-Created-At",
        HeaderValue::from(manifest.created_at),
    );
}
...
async fn get_bundle(
    ...
    let bundle_path = temp_dir.path().join("export.ministr-index");
    ...
    let _ = headers.insert(
        ...
        HeaderValue::from_static("attachment; filename=\"index.ministr-index\""),
    );
...
mod tests {
    ...
    fn version_headers_populated() {
        ...
        assert_eq!(headers.get("X-Ministr-Bundle-Version").unwrap(), "abc123");
        assert_eq!(headers.get("X-Ministr-Source-Commit").unwrap(), "deadbeef");
        assert_eq!(headers.get(header::ETAG).unwrap(), "\"abc123\"");
        assert_eq!(headers.get("X-Ministr-Created-At").unwrap(), "1700000000");
    }
    ...
    fn version_headers_none_fields() {
        ...
        assert!(headers.get("X-Ministr-Bundle-Version").is_none());
        assert!(headers.get("X-Ministr-Source-Commit").is_none());
        assert!(headers.get(header::ETAG).is_none());
        // created_at always present
        assert!(headers.get("X-Ministr-Created-At").is_some());
    }
}
