//! Agent-to-Agent (A2A) protocol support for ministr.
//!
//! Implements the A2A protocol (Google/Linux Foundation) so other AI agents
//! can discover ministr's code intelligence capabilities via an Agent Card and
...
use ministr_core::session::SessionRegistry;

// ---------------------------------------------------------------------------
// Agent Card types
// ---------------------------------------------------------------------------

/// A2A Agent Card describing ministr's capabilities.
...
/// Build the ministr Agent Card.
#[must_use]
pub fn ministr_agent_card() -> AgentCard {
    AgentCard {
        name: "ministr".to_string(),
...
/// Return the ministr Agent Card.
async fn agent_card_handler() -> impl IntoResponse {
    Json(ministr_agent_card())
}
...
async fn dispatch_symbols(
    ...
    let filter = ministr_core::storage::SymbolFilter {
...
mod tests {
    ...
    fn agent_card_has_required_fields() {
        let card = ministr_agent_card();
        assert_eq!(card.name, "ministr");
