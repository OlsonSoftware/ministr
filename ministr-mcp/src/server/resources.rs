//! MCP resource and completion handlers for the ministr server.
//!
//! These `impl MinistrServer` methods handle resource reading (`mcp://server-card.json`,
//! `ministr://status`, `ministr://corpus/{path}`) and completion requests for section IDs
//! and corpus paths.
...
impl MinistrServer {
    ...
    pub(super) async fn read_status_resource(&self) -> Result<ReadResourceResult, McpError> {
        ...
        let (session_id_str, delivered_count, budget_status) = {
            ...
            (
                ...
                entry.session.delivered_count(),
                entry.budget.usage_status(),
            )
        };
        let session_count = reg.session_count();
        let session_ids = reg.session_ids();
        drop(reg);
    ...
    /// Build the `ministr://corpus/{path}` resource content.
    pub(super) async fn read_corpus_resource(
        ...
        let uri = format!("ministr://corpus/{path}");
