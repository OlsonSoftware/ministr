...
///
/// In a production system this would render a consent page. For the ministr
...
mod tests {
    ...
    fn default_config_has_required_scopes() {
        ...
        assert!(
            config
                .scopes_supported
                .contains(&"ministr:read".to_string())
        );
        assert!(
            config
                .scopes_supported
                .contains(&"ministr:write".to_string())
        );
        assert!(
            config
                .scopes_supported
                .contains(&"ministr:bundle:read".to_string())
        );
        assert!(
            config
                .scopes_supported
                .contains(&"ministr:bundle:write".to_string())
        );
    ...
    async fn validate_token_with_scope_matching() {
        ...
        store.tokens.write().await.insert(
            ...
            AccessToken {
                token: token_val.to_string(),
                client_id: "client-1".into(),
                scope: "ministr:read ministr:bundle:read".into(),
                expires_at: epoch_now() + 3600,
            },
        );
        ...
        let result = store
            .validate_token_with_scope(token_val, "ministr:bundle:read")
            .await;
        assert_eq!(result, Some("client-1".to_string()));

        // Also has ministr:read.
        let result = store
            .validate_token_with_scope(token_val, "ministr:read")
            .await;
        assert_eq!(result, Some("client-1".to_string()));
    }
    ...
    async fn validate_token_with_scope_missing() {
        ...
        // Does NOT have ministr:bundle:read — should fail.
        let result = store
            .validate_token_with_scope(token_val, "ministr:bundle:read")
            .await;
        assert_eq!(result, None);
    ...
    async fn validate_token_with_scope_expired() {
        ...
        store.tokens.write().await.insert(
            ...
            AccessToken {
                token: token_val.to_string(),
                client_id: "client-1".into(),
                scope: "ministr:bundle:read".into(),
                expires_at: epoch_now().saturating_sub(100),
            },
        );
