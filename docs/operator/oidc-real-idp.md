...
F5.2-c callback path. The fix column maps to a concrete next step.
...
| `id_token carries no email claim` | The IdP's `email` scope wasn't granted or the claim is namespaced. | Confirm the `email` scope is admitted by the IdP's client config. If the IdP issues a non-standard claim name, point `email_claim` at it (F5.2-c reads this column; the OIDC SDK exposes the standard `email` claim via accessor, so non-standard names won't surface in v0 — a v1 chunk extends the callback to honour the column). |
...
sign-in, SAML if F5.1 is wired, etc).
