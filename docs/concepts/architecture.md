...
- The first client auto-spawns the daemon as a detached process. It
  deliberately outlives its spawner: closing your editor leaves indexes
...
- The daemon owns the state: per-corpus indexes under `~/.ministr`, run
  ...
  deterministic hash of the canonicalized path set, so every client can
  agree about which index a project maps to.
- Any other local client can poll the same
