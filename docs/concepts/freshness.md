...
An editor that rewrites a file with an old mtime, a git checkout that
restores timestamps, a build tool that touches without changing — none of
these can make ministr claim "up to date" falsely or "behind" spuriously.
Internally, indexing uses mtime and size only as an accelerator to decide
what to re-check; the per-file content hash always has the final word, and
reporting never takes the shortcut at all.
...
- Agents feel it as `coherence_alerts` in tool responses: when a file
  changes after its content was delivered, the next response says so, and
  re-reading yields the delta.
