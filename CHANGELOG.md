...
## [0.4.0](https://github.com/OlsonSoftware/ministr/releases/tag/v0.4.0) - 2026-05-17

### Added
- Terraform variable files (`.tfvars`) are now indexed as HCL for code-aware ingestion.

### Changed
- Release CI now builds only on the release-bump commit to avoid redundant matrix runs.
- Release-request automation now waits while a release is prepared but not yet tagged.
- Release PR authoring is now handled by the GitHub Copilot cloud agent flow.

### Breaking changes — migration
- Release authoring automation moved from `scripts/ci/release_pr.py` to `.github/workflows/release-automation.yml` (GitHub Copilot cloud agent managed flow).
