...
## [0.7.1](https://github.com/OlsonSoftware/ministr/releases/tag/v0.7.1) - 2026-06-13

### Added
- No user-facing additions in this patch release.

### Changed
- MCP execution flow no longer relies on elicitation, so tool handling stays non-interactive end-to-end.

### Fixed
- Fixed MCP behavior where tools could block waiting for human-form elicitation.

### Removed
- Removed the MCP elicitation subsystem from `ministr-mcp`.

## [0.7.0](https://github.com/OlsonSoftware/ministr/releases/tag/v0.7.0) - 2026-06-12

### Breaking
- **The repository split into an MIT local stack and a proprietary cloud sibling.** The six MIT crates (`ministr-core`, `ministr-api`, `ministr-daemon`, `ministr-mcp`, `ministr-cli`, `ministr-app`) now build a complete local binary on their own; the cloud stack (multi-tenant serve, OAuth/orgs/billing, enterprise SSO/SIEM/licensing) moved to a private sibling workspace behind the public `CloudRouterMounter` seam in `ministr-api`. `cargo build -p ministr-cli` produces the full local product with no cloud dependencies. Signed release artifacts are now built by the private repo's CI and published to this repo's Releases — `install.sh` and Homebrew are unchanged.

### Added
- **Hybrid retrieval is on by default for code corpora.** A new zero-model AST-native sparse encoder (BM25F-style weighting over AST roles — symbol name, doc comment, signature, body — with subtoken analysis and corpus IDF derived live from the inverted-index postings) pairs with dense embeddings via reciprocal-rank fusion. On the deterministic retrieval gate this lifts nDCG@10 from .666 to .798 and MRR from .690 to .875 on code queries, costs no model download, and encodes ~10k entries/s. Tune or disable per corpus with the new `[corpus] sparse_weight` knob in `.ministr.toml` (default 0.6 for code, 0 for docs corpora); a SPLADE encoder remains available via `sparse_encoder = "splade"`.
- **`ministr_run` — a recorded shell for agents.** A new exec engine spawns commands with bounded capture and persistent run records, renders token-lean digests back to the agent, ingests finished run logs into the corpus, and (opt-in via `ministr init --exec-only`) steers agents away from raw shell so every command they run is captured and reviewable. The daemon hosts the engine; the desktop app gains a Run Console with a live timeline.
- **Code intelligence toward LSP parity, with a parity gate.** Cross-file occurrence indexing (click any token to its symbol), `ministr_diagnostics` (language-agnostic structured diagnostics), type-hierarchy-aware references (`through_implementors`), diff-aware blast radius (`base..head` on `ministr_impact`), git blame on `ministr_definition`, and call/implements/uses edge graphs extended across JS/TS, Python, Java, Kotlin, C#, Swift, Ruby, PHP, Go, and Scala — all enforced by an always-on LSP-equivalence parity matrix in CI.
...
- The vector index is rebuildable from a SQLite source of truth: every ingest persists the exact indexed vector (ADR 0001 D4), and `serve` rebuilds the ANN index from storage when the cache is invalid — index drift is structurally impossible.
- A dedicated dynamically-batched `EmbeddingService` and an `IngestionScheduler` with bounded concurrency and per-corpus exclusion replace the old one-at-a-time indexing path; tree-sitter parsing moved to a dedicated rayon pool off the Tokio runtime.
...
- **Desktop app rebuilt around trust.** The app is a new "Clear Glass" design: a Trust Panel home (one plain-English trust row per project, worst first), a Project Mirror (the file tree exactly as your AI sees it, hash-verified), a three-beat connect flow, full keyboard navigation, light/dark/system theming, and honest daemon-down states. Indexing is now a live visual instrument — a segmented per-phase progress track with real counts, the file being read right now, throughput, and an honest time-remaining estimate (shown only once the rate is stable, hidden during stalls).
- The docs site was rewritten from scratch against the actual source (44 pages), and the marketing pages moved to the v2 design with the cross-language bridge and measured benchmark figures.

### Changed
- Retrieval scoring is now raw cosine similarity end-to-end — the old per-kind weight table was deleted after the deterministic gate showed it strictly hurt quality (nDCG .573 → .603 on its removal).
- The retrieval eval gate is byte-deterministic: an exact-scan index for evals, CPU-pinned inference (`MINISTR_COREML=0`), and intra-batch embedding dedup make `just eval-quality` hash-stable across runs, so quality regressions are now mechanically detectable.
- A no-op re-index (nothing changed on disk) now skips loading the embedding model entirely — `ministr index` on an unchanged corpus dropped from ~3.1s to ~0.03s (ONNX path); the daemon and CLI share one embed-cache path with intra-batch dedup.
...
- The macOS app adopted React Compiler 1.0 (build-time auto-memoization) and the Iconoir icon system, and ships a real branded app icon.

### Fixed
- `.ministr.toml` `[corpus] ignore` patterns are now actually enforced at ingest, and watcher events respect the full exclusion stack (builtins + gitignore + `[corpus] ignore`) — previously a watched file edit could re-index content the walker would have excluded.
- The live file watcher keeps the sparse (hybrid) index coherent: modified files re-embed their sparse postings and deleted files tombstone them, so hybrid search can no longer return dead hits from removed files.
- Matryoshka two-stage rerank is no longer starved: ingest stores full-dimension vectors alongside the truncated search dimension.
- Zero and non-finite embedding vectors are refused at ingest (they poisoned cosine search), with a transient-retry before skipping, and a degenerate vector now skips one section instead of rolling back the whole corpus.
- Cross-file references resolve between `.tsx` and `.ts` files (JS/TS are one reference family), and `ministr_references` honors its `ref_kind` filter on the daemon backend.
...
- Dropped the Help submenu on macOS 26 (Tahoe) to dodge an AppKit menu-bar crash at launch.
- A corpus with zero indexed files is never shown as "indexed/Ready", and corpus indexing is serialized to prevent a runtime-starvation hang.
