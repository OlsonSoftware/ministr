...
pub async fn corpus_freshness(
    corpus_id: String,
) -> Result<ministr_api::corpus::FreshnessResponse, CommandError> {
    ministr_api::client::DaemonClient::new()
        .corpus_freshness(&corpus_id)
        .await
        .map_err(Into::into)
}
...
/// FL7 — diff-aware blast radius for the Explore "Changes" lens. Thin
/// passthrough to the daemon `/diff-impact` route, which resolves the repo from
/// the corpus's local root and runs the whole composition server-side (git diff
/// → enclosing symbols + blame → union impact). One round-trip — the local git
/// + N+1 impact calls now live in `ministr_core::QueryService::compute_diff_impact`.
#[tauri::command]
pub async fn diff_impact(
    corpus_id: String,
    range: String,
    max_depth: Option<u32>,
) -> Result<ministr_api::query::DiffImpactResponse, CommandError> {
    ministr_api::client::DaemonClient::new()
        .diff_impact(&corpus_id, &range, max_depth, None)
        .await
        .map_err(Into::into)
}
