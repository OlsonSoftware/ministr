...
// ── FL7 — the Explore "Changes" lens: diff-aware blast radius + blame. ───────

/// One contributor's share of a changed symbol's lines.
#[derive(Serialize)]
...
/// A symbol the diff range touched (the seed set), with authorship.
#[derive(Serialize)]
...
/// Diff-aware blast radius for the Explore "Changes" lens.
#[derive(Serialize)]
pub struct DiffImpactOut {
    pub range: String,
    ...
    pub risk: String,
    pub impacted: Vec<ImpactedOut>,
}

/// Whether a path looks like a test file (mirrors the core/MCP heuristic) —
/// used to count distinct test files in the union blast radius.
...
/// Heuristic risk for the union blast radius (mirrors the core `compute_risk`).
fn union_risk(symbols: usize, files: usize, tests: usize) -> &'static str {
    let score = symbols
...
/// Cap on the diff seed set (responsiveness).
const MAX_DIFF_SEEDS: usize = 60;
/// Cap on unioned impacted nodes returned.
const MAX_DIFF_IMPACTED: usize = 400;

/// Map a parsed diff's changed lines to the enclosing indexed symbols (the seed
/// set), attaching per-symbol git blame. Returns the seeds plus the count of
/// files that yielded a symbol.
async fn collect_diff_seeds(
    client: &ministr_api::client::DaemonClient,
    corpus_id: &str,
    changed: &[ministr_core::git::ChangedFile],
) -> (Vec<ChangedSymbolOut>, usize) {
    use std::collections::HashSet;
    let mut seeds: Vec<ChangedSymbolOut> = Vec::new();
    ...
    for file in changed {
        ...
        let Ok(resp) = client.symbols(corpus_id, &req).await else {
            continue;
        };
        ...
        for s in resp.symbols {
            ...
            if touched && seed_ids.insert(s.id.clone()) {
                file_hit = true;
                let blame = ministr_core::git::blame::blame_range(
        ...
        if seeds.len() >= MAX_DIFF_SEEDS {
            break;
        }
    }
    seeds.sort_by(|a, b| a.file.cmp(&b.file).then(a.line.cmp(&b.line)));
    seeds.truncate(MAX_DIFF_SEEDS);
    (seeds, changed_files)
}

/// Union the incoming blast radius across the seed symbols (one daemon `impact`
/// call per seed), deduping impacted nodes by id (shallowest depth wins).
async fn union_diff_impact(
    client: &ministr_api::client::DaemonClient,
    corpus_id: &str,
    seeds: &[ChangedSymbolOut],
    depth: u32,
) -> Vec<ImpactedOut> {
    use std::collections::HashMap;
    let mut map: HashMap<String, ImpactedOut> = HashMap::new();
    for seed in seeds {
        let Ok(resp) = client
            .impact(
                corpus_id,
        ...
        for c in resp.callers {
            map.entry(c.symbol_id.clone())
                .and_modify(|e| {
                    if c.depth < e.depth {
                        e.depth = c.depth;
                    }
                })
                .or_insert(ImpactedOut {
    ...
    let mut impacted: Vec<ImpactedOut> = map.into_values().collect();
    ...
    impacted.truncate(MAX_DIFF_IMPACTED);
    impacted
}

/// FL7 — diff-aware blast radius + authorship for the Explore "Changes" lens.
///
/// Resolves a git `range` (e.g. `main..HEAD`) to the indexed symbols it touched
/// (the seed set), attaches per-symbol git blame, and unions their impact (what
/// the branch can break). The GUI mirror of the MCP `ministr_impact` `range`
/// op: composes local git (`ministr_core::git`) with the daemon's `symbols` +
/// `impact` endpoints. Results are bounded for responsiveness.
#[tauri::command]
pub async fn diff_impact(
...
) -> Result<DiffImpactOut, CommandError> {
    use std::collections::HashSet;

    // 1. Resolve the diff's changed files + head-side line ranges (local git).
    let changed = ministr_core::git::diff::changed_lines(std::path::Path::new(&repo_path), &range)
        .map_err(|e| CommandError::invalid_input(format!("git range '{range}': {e}")))?;

    let client = ministr_api::client::DaemonClient::new();
    let depth = max_depth.unwrap_or(3);

    // 2. Changed lines → enclosing symbols (+ blame). 3. Union their impact.
    let (seeds, changed_files) = collect_diff_seeds(&client, &corpus_id, &changed).await;
    let impacted = union_diff_impact(&client, &corpus_id, &seeds, depth).await;
    ...
    let (n_sym, n_file, n_test) = (impacted.len(), files.len(), tests.len());
    let risk = union_risk(n_sym, n_file, n_test).to_string();

    Ok(DiffImpactOut {
        range,
        changed_files,
        changed_symbols: seeds,
        impacted_symbols: n_sym,
        impacted_files: n_file,
        impacted_tests: n_test,
        risk,
        impacted,
    })
}
