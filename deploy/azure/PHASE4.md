...
- [x] **Chunk 5 — Right-size worker post-streaming** — added `ministr-azure:jobMemory: 4Gi` to `Pulumi.prod.yaml` (down from the 8 GiB default in `index.ts`). Initially kept `jobCpu` at the default 4; **operator deploy uncovered an Azure constraint** (`ContainerAppInvalidResourceTotal`) — ACA Consumption profile only accepts a closed set of CPU/memory pairs, and 4 vCPU forces 8 GiB. Resolved by also pinning `ministr-azure:jobCpu: "2"`: 2 vCPU + 4 GiB is the closest valid pair below the default. Cost win at the corrected spec: ~$1/h-when-running on 2 vCPU / 4 GiB vs ~$4/h on the 4 vCPU / 8 GiB default — 4× active-time win. Tradeoff: embedding throughput roughly halves vs 4 vCPU (small repo ~30s → ~60s); idle remains $0/mo via the KEDA 0-replica scaler. Operator-side `just azure-demo` is the canonical verification (`pulumi up` + push + roll + `azure-smoke`). *(Post-chunk: `phase4-smoke` collapsed into `azure-smoke`, the canonical ever-evolving smoke `just azure-demo` calls at its tail. One command to test any phase.)*
