...
- [ ] **Cloud worker streams via `persist_every`** — wire `cmd_indexer_worker` / `run_corpus_ingestion` to call `IngestionPipeline::with_corpus_dir(ctx.corpus_dir)` and pass `BatchIngestionConfig { persist_every: Some(4), batch_size: 4 }`. The plumbing landed in chunk 4; this is the opt-in flip for the cloud worker only (keeps local `ministr index` on the bundle-at-end path).
