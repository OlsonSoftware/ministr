...
//!
//! Ensures only one ministr process owns the HNSW index for a given corpus.
...
/// The role this ministr instance should assume.
...
///
/// Tries to acquire an exclusive file lock on `{corpus_dir}/ministr.lock`.
...
/// `{corpus_dir}/ministr.port`, and returns [`InstanceRole::Primary`].
///
...
pub fn acquire_role(corpus_dir: &Path, corpus_paths: &[String]) -> miette::Result<InstanceRole> {
    let lock_path = corpus_dir.join("ministr.lock");
    let port_path = corpus_dir.join("ministr.port");

    let lock_file = File::create(&lock_path)
        .into_diagnostic()
        .wrap_err("failed to create ministr.lock")?;
    ...
    match lock_file.try_lock() {
        ...
        Err(TryLockError::WouldBlock) => {
            ...
            let lock_file = File::create(&lock_path)
                .into_diagnostic()
                .wrap_err("failed to re-create ministr.lock")?;
            #[allow(clippy::incompatible_msrv)]
            lock_file
                .try_lock()
                .into_diagnostic()
        ...
        Err(TryLockError::Error(e)) => Err(e)
            .into_diagnostic()
            .wrap_err("failed to acquire ministr.lock"),
    }
}
...
fn resolve_port(corpus_paths: &[String], port_path: &Path) -> miette::Result<u16> {
    ...
    fs::write(port_path, port.to_string())
        .into_diagnostic()
        .wrap_err("failed to write ministr.port")?;
...
/// Read `ministr.port` with retries (the primary may still be writing it).
fn read_port_file_with_retry(port_path: &Path) -> miette::Result<String> {
    for attempt in 0..10 {
        match fs::read_to_string(port_path) {
            ...
            Err(e) => {
                warn!(error = %e, "failed to read ministr.port");
            }
        }
    ...
    miette::bail!("timed out waiting for primary to write ministr.port — primary may have crashed")
}
