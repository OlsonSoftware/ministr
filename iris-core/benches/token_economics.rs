...
//! concatenation (grep/cat workflow) against iris-style targeted retrieval.
//!
//! Measures token counts at various codebase sizes to quantify the savings
//! iris provides over naive approaches.
...
/// A targeted extract (what iris would deliver for "how does auth validation work?").
...
fn bench_iris_targeted_tokens(c: &mut Criterion) {
    c.bench_function("iris_targeted_extract", |b| {
    ...
    for size in [5, 20, 50, 100] {
        ...
        group.bench_with_input(BenchmarkId::new("files", size), &codebase, |b, files| {
            b.iter(|| {
                ...
                let iris = count_tokens(TARGETED_EXTRACT);
                1.0 - (iris as f64 / raw as f64)
            });
        });
    }
