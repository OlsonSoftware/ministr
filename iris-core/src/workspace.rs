...
//!
//! ```no_run
//! use iris_core::workspace::detect_workspace;
//! use std::path::Path;
//!
...
mod tests {
    ...
    fn detect_cargo_workspace() {
        ...
        create_dirs(root, &["iris-core/src", "iris-mcp/src", "iris-cli/src"]);

        std::fs::write(
            root.join("Cargo.toml"),
            r#"
[workspace]
members = ["iris-core", "iris-mcp", "iris-cli"]
resolver = "2"
"#,
        )
        .unwrap();
        ...
        for member in &["iris-core", "iris-mcp", "iris-cli"] {
            std::fs::write(
        ...
        assert!(ws.members.contains(&root.join("iris-core")));
        assert!(ws.members.contains(&root.join("iris-mcp")));
        assert!(ws.members.contains(&root.join("iris-cli")));
    }
