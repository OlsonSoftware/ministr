...
//!     Path::new("/repo"),
//!     &[Path::new("/repo/iris-core").to_path_buf()],
//! );
//! assert_eq!(graph.dir_prefix_for_crate("iris_core"), Some("iris-core"));
//! ```
...
pub struct PackageInfo {
    /// The package name as declared in `Cargo.toml` (e.g., `"iris-core"`).
    pub name: String,
    /// The crate name used in Rust source (hyphens replaced with underscores, e.g., `"iris_core"`).
    pub crate_name: String,
    /// Directory prefix relative to the workspace root (e.g., `"iris-core"`).
...
impl PackageGraph {
    ...
    ///
    /// The crate name uses underscores (e.g., `"iris_core"`), as it appears
