...
///
/// ```
/// use iris_core::code::{AstItem, ItemKind};
///
/// let item = AstItem {
...
///
/// ```
/// use iris_core::code::{AstParser, ItemKind, walk_top_level_items};
///
/// let mut parser = AstParser::new();
...
";
    ...
    // C1.3: Parse real iris-core source files
    ...
    fn parse_config_rs_identifies_structs_and_impls() {
        ...
        // config.rs should contain IrisConfig struct and PrefetchConfig struct
        let struct_names: Vec<&str> = items
            ...
            .collect();
        assert!(
            struct_names.contains(&"IrisConfig"),
            "expected IrisConfig struct, found: {struct_names:?}"
        );
        assert!(
        ...
        // Should have impl blocks for IrisConfig
        let impl_names: Vec<&str> = items
            ...
            .collect();
        assert!(
            impl_names.contains(&"IrisConfig"),
            "expected IrisConfig impl, found: {impl_names:?}"
        );
