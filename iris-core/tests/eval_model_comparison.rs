...
//! IRIS_EVAL_MODELS="all-MiniLM-L6-v2" just bench-model # single model
//! ```
...
async fn compare_embedding_models() {
    ...
    if let Ok(path) = std::env::var("IRIS_EVAL_OUTPUT") {
...
/// Resolve which models to benchmark. Respects `IRIS_EVAL_MODELS` env var
/// (comma-separated model names) or falls back to `DEFAULT_MODELS`.
fn resolve_model_specs() -> Vec<ModelSpec> {
    if let Ok(env_models) = std::env::var("IRIS_EVAL_MODELS") {
        env_models
            .split(',')
            .map(str::trim)
            .filter(|s| !s.is_empty())
